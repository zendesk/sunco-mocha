...
const singleRun = async (
  mocha,
  {exit, changed, watchIgnore: ignore},
  fileCollectParams
) => {
  let entryFiles = collectFiles(fileCollectParams);
  debug('found %d test files', entryFiles.length);
  if (changed) {
    const moduleMap = ModuleMap.create({entryFiles, ignore});
    entryFiles = moduleMap.findAffectedFilesForChangedFiles().entryFiles;
    debug('--changed requested; running %d test files', entryFiles.size);
  }
  mocha.files = entryFiles;
