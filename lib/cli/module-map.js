...
const CACHE_DIR_FALLBACK = path.join(
  process.cwd(),
  'node_modules',
  '.cache',
  'mocha'
...
class ModuleMap extends Map {
  ...
  constructor({
    ...
    super();
    this.cwd = cwd;
    this.cacheDir = cacheDir || ModuleMap.findCacheDir(cwd);
  ...
  static findCacheDir(cwd = process.cwd()) {
    return (
      findCacheDir({name: 'mocha', create: true, cwd}) || CACHE_DIR_FALLBACK
    );
  }
  ...
  get fileEntryCache() {
    return this._fileEntryCache;
  }

  get moduleMapCache() {
    return this._moduleMapCache;
  }
  ...
  init({reset = false, force = false} = {}) {
    ...
    const nodes = this.getAll(this.popChangedFiles());
  ...
  isEntryFile(filename) {
    return this.entryFiles.has(filename);
  }
  ...
  persistFileEntryCache({filenames = this.files} = {}) {
    this.fileEntryCache.normalizeEntries([...filenames]);
    this.fileEntryCache.reconcile(true);
   ...
   * @param {string} file - Filepath to mark changed
   * @returns {ModuleMap}
   */
  markFileAsChanged(file) {
    this.fileEntryCache.removeEntry(file);
    return this;
  }
  ...
  findAffectedFilesForChangedFiles({knownChangedFiles = []} = {}) {
    knownChangedFiles = new Set(
      castArray(knownChangedFiles).map(filename =>
    ...
    const changedFilenames = this.popChangedFiles();

    if (changedFilenames.size) {
      let changedNodes = this.getAll(changedFilenames);

      /* istanbul ignore next */
      if (changedNodes.size !== changedFilenames.size) {
        debug(
          'found %d nodes for %d changed files; attemping sync from disk',
          changedNodes.size,
          changedFilenames.size
        );
        this.mergeFromCache({destructive: true});
        changedNodes = this.getAll(changedFilenames);
        if (changedNodes.size !== changedFilenames.size) {
  ...
  /**
   * From the known files, return a list of files that have changed and remove them from the list of changed files.
   * Effectively "pops" them off of the list by persisting the file entry cache.
   */
  popChangedFiles() {
    const files = new Set(this.fileEntryCache.getUpdatedFiles([...this.files]));
    debug(
