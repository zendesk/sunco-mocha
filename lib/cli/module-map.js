...
class ModuleMap extends Map {
   ...
   */
  get directories() {
    return new Set([...this.files].map(filename => path.dirname(filename)));
  }
   ...
   * @param {string[]|Set<string>} filepaths
   * @returns {string[]}
   */
  filterEntryFiles(filepaths) {
    return [...filepaths].filter(filepath =>
      this.entryFiles.has(path.resolve(this.cwd, filepath))
    );
  }
   ...
   */
  findAffectedFiles({knownChangedFiles = []} = {}) {
    knownChangedFiles = new Set(
      castArray(knownChangedFiles).map(filename =>
        path.resolve(this.cwd, filename)
      )
    );
    knownChangedFiles.forEach(markChangedFile => {
      this.markFileAsChanged(markChangedFile);
    });
    const changedFiles = this.popChangedFiles();
    if (changedFiles.size) {
      const nodes = [...changedFiles]
