...
const createWatcher = (
  ...
  watchIgnore = [...watchIgnore, ModuleMap.findCacheDir()];
  ...
  const testFiles = collectFiles(fileCollectParams);
  debug('found test files: %s', testFiles);

  const cache = ModuleMap.create({
  ...
  const watcher = chokidar.watch(
    watchFiles.length ? watchFiles : [...cache.files, ...cache.directories],
    {
      ignored: watchIgnore,
      ignoreInitial: true
    }
  ...
  const findChangesAndRerun = filename => {
    if (!filename) {
      ...
      const {entryFiles, allFiles} = cache.findAffectedFilesForChangedFiles({
